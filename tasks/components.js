...
module.exports = {
  js: function () {
    gulp.src([config.components.srcJs])
    // Concatenate includes
    .pipe(include())
    // Transpile
    .pipe(babel())
    .pipe(concat('components.js'))
    .pipe(gulp.dest(config.components.distJs))
    // Optimize and minify
    .pipe(uglify())
    // Append suffix
    .pipe(rename({
  ...
  css: function () {
    gulp.src(config.components.srcCss)
    ...
    .pipe( lineec() )
    .pipe(stylefmt())
    .pipe(gulp.dest(config.components.distCss))
    .pipe( postcss([
